...
public class Section {
	...
	public static final int PREVIOUSLY_ENROLLED = 1;
	public static final int PREREQ_NOT_SATISFIED = 2;
	public static final int SECTION_FULL = 3;
	...
	public int enroll(Student s) {
		...
		if (s.isCurrentlyEnrolledInSimilar(this) || 
			...
			return Section.PREVIOUSLY_ENROLLED;
		}
		...
		if (c.hasPrerequisites()) {
			Collection prereq = c.getPrerequisites();
			Iterator iter = prereq.iterator();
			while (iter.hasNext()) {
				Course pre = (Course) iter.next();

				// See if the Student's Transcript reflects
				// successful completion of the prerequisite.

				if (!transcript.verifyCompletion(pre)) {
					return Section.PREREQ_NOT_SATISFIED;
				}
			}
		}
	...
	public void displayStudentRoster() {
		...
		Collection students = enrolledStudents.values();
