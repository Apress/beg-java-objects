...
public class MainFrame extends JFrame {
		...
		aListener = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (currentUser.isCurrentlyEnrolledInSimilar(
					...
					int success = selected.enroll(currentUser);
	...
	private Section[] sortSections(HashMap sections) {
		// We'll transfer the contents of the HashMap to a TreeMap, which
		// automatically orders entries in ascending key order.

		TreeMap sortedMap = new TreeMap(sections);
		
		// Explain ...

		// Note cast to a Section array.
		return (Section[]) (sortedMap.values().toArray(new Section[1]));
	}
